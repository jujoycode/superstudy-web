...
import _, { flatten, map, sortBy } from 'lodash'
...
export const ActivityV3DetailPage: React.FC<ActivityV3DetailPageProps> = () => {
  ...
  const teacherActivityV3GroupIds = flatten(activityv3?.teacherActivityV3s?.map((tav3) => tav3.groupIds) || [])

  const { data: groupByIdsData } = useGroupsFindAllByIds(
    { ids: teacherActivityV3GroupIds },
    { query: { enabled: !!teacherActivityV3GroupIds.length } },
  )

  const groups = groupByIdsData?.groupsByIds || []
  ...
  const studentGroups = useMemo(() => {
    if (!data) return []
    return _.chain(data)
      .filter((el) => !activityv3?.isJointActivity || map(activityv3.groupActivityV3s, 'groupId').includes(el.groupId))
      .uniqBy('user.id')
      .sortBy('groupId')
      .value()
  }, [data])
          ...
          <div className="3xl:px-30 3xl:py-20 h-full overflow-y-auto bg-white p-2 md:px-10 md:py-5">
            ...
            <div className="flex flex-col rounded border-2 border-zinc-800">
              ...
              <div className="flex items-center px-10 py-5">
                ...
                {activityv3?.isJointActivity ? (
                  <div className="flex flex-col space-y-2">
                    {activityv3.teacherActivityV3s?.map((tav3) => (
                      <div className="flex items-center space-x-2" key={tav3.id}>
                        <div className="bg-primary-orange-100 flex items-center space-x-2 rounded-lg px-3 py-2 whitespace-pre">
                          ...
                          <div className="gray-700 text-15 font-medium">{tav3.user?.name}</div>
                        </div>
                        {tav3.groupIds.map((groupId) => (
                          <div
                            className="flex items-center space-x-2 rounded-lg bg-gray-50 px-3 py-2 whitespace-pre"
                            key={tav3.userId + 'group' + groupId}
                          >
                            <div className="gray-700 text-15 font-medium">
                              {groups?.find((el) => el.id === groupId)?.name}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="relative">
                    <button
                      ...
                      onClick={() =>
                        ...
                        (activityv3.isJointActivity &&
                          map(activityv3.teacherActivityV3s, 'userId').includes(me?.id || 0))
                  ...
                  >
                    {_.chain(activityv3?.groupActivityV3s || [])
                      .filter(
                        (el) =>
                          !activityv3?.isJointActivity ||
                          !!activityv3?.teacherActivityV3s
                            ?.find((el) => el.userId === me?.id)
