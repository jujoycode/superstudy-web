...
export function TeacherInfoPage() {
  ...
  return (
    <Grid col={12} className="bg-white">
      <GridItem colSpan={1} empty />
      <GridItem colSpan={10}>
        <ResponsiveRenderer mobile={<TopNavbar title={`${t('my_information')}`} left={<BackButton />} />} />

        <ScrollArea className="h-screen">
          <Section className="flex flex-col bg-white p-0">
            {isUpdateMeLoading && <Blank />}

            {/* 교원증 영역 */}
            <div className="mt-16 flex h-[428px] justify-center">
            ...
            {/* 내 정보 영역 */}
            {!isDeleteMe && (
              <div className="mt-4 px-4 py-4 md:px-10 md:py-5">
