...
export const IBTeacherReferenceDetailPage = () => {
  const { push } = useHistory()
  const { id } = useParams<{ id: string }>()
  const { data } = useReferenceGetById(Number(id))
            ...
            <div className="flex grow flex-col py-6">
              <div className="flex min-h-[720px] flex-col justify-between rounded-xl bg-white p-6">
                ...
                <footer className={`flex flex-row items-center justify-start`}>
                  <>
                    <ButtonV2 size={40} variant="solid" color="gray100" onClick={() => push(`/teacher/ib/reference`)}>
                      목록 돌아가기
                    </ButtonV2>
                  </>
                </footer>
