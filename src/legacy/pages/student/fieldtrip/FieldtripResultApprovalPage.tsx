...
import { useParentFieldtripResultApprove } from '@/legacy/container/parent-fieldtrip-result-approve'
...
export function FieldtripResultApprovalPage() {
  ...
  const {
    ...
    openSignModal,
    isShowSignModal,
  } = useParentFieldtripResultApprove({
    uuid,
    sigPadData,
  })
  ...
  const isSigned = fieldtrip?.parentResultSignature
  ...
  return (
    <>
      ...
      {isError && <ErrorBlank text={errorMessage} />}
      <TopNavbar
        title="체험학습 결과보고서"
        ...
        {type === 'SUBURBS' && (
          <>
            ...
            <div>
              ...
              <div className="w-full border border-black p-3 whitespace-pre-line">{fieldtrip?.resultTitle}</div>
            </div>
            <div>
              <div className="text-base">*결과보고서 내용</div>
              <div className="w-full border border-black p-3 whitespace-pre-line">{fieldtrip?.resultText}</div>
            </div>
      ...
      <div className={isShowSignModal ? '' : 'hidden'}>
        ...
        <BottomFixed className="-bottom-4 z-100 rounded-xl">
          <div className="absolute top-2 right-3" onClick={hideSignModal}>
            <CloseButton
              onClick={() => {
                hideSignModal()
                clearSignature()
