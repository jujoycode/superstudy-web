          ...
          {weekdays.map((day, index) => (
            <Typography
              ...
              className={`text-center ${
                index === 0 ? 'text-old-primary-red-400' : index === 6 ? 'text-ib-blue-400' : 'text-gray-500'
              }`}
            >
              {day}
            </Typography>
        ...
        <div className="relative grid w-[248px] grid-cols-7 gap-y-1">
          {dates.map(({ date: calendarDate, isCurrentMonth }) => (
            <div key={calendarDate.toISOString()} className={`relative flex items-center justify-center`}>
              <Typography
                variant="body3"
                className={`z-10 flex h-8 w-8 cursor-pointer items-center justify-center font-medium ${isCurrentMonth ? 'text-gray-900' : 'text-gray-400'} ${isSameDay(calendarDate, new Date()) ? 'border-primary-400 rounded-full border' : ''} ${
