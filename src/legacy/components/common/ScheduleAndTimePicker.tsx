        ...
        <div className="relative grid w-[248px] grid-cols-7 gap-y-1">
          {dates.map(({ date: calendarDate, isCurrentMonth }) => (
            <div key={calendarDate.toISOString()} className={`relative flex items-center justify-center`}>
              <Typography
                variant="body3"
                className={`z-10 flex h-8 w-8 cursor-pointer items-center justify-center font-medium ${isCurrentMonth ? 'text-primary-gray-900' : 'text-primary-gray-400'} ${isSameDay(calendarDate, new Date()) ? 'border-primary-400 rounded-full border' : ''} ${
