...
import EssayOverviewPanel from '@/legacy/components/ib/overview/TOK/Essay/EssayOverviewPanel'
import {
...
export default function EssayView({ grade, klass, ibType }: EssayGetSubmissionStatusCountParams) {
  ...
  const { mutate: notiMutate, isLoading: notiLoading } = useIBEssayNotSubmittedNotification({
    params: {
      type: EssayUnsubmitNotificationType.TOK_ESSAY,
      grade,
      klass,
    },
    onError: (error) => {
    ...
    <div>
      ...
      {status === 'SUBMITTED' && (
        <div className="flex flex-col gap-4">
          ...
          <EssayOverviewPanel
            title="진행중"
            goDetailPage={(studentId, essayId) => push(`/teacher/ib/tok/essay/${studentId}/detail/${essayId}`)}
            data={students}
          />
        </div>
      )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <EssayOverviewPanel
            ...
            goDetailPage={(studentId, essayId) =>
              push(`/teacher/ib/tok/essay/${studentId}/detail/${essayId}`, { type: 'evaluation' })
            }
            data={students}
            type="NOT_EVALUATED"
          />
