...
import { ResponseIBTokOutlineSubmissionStatusDto } from '@/legacy/generated/model'

interface OverviewPanelProps {
  goDetailPage?: (studentIbId: number, outlineId: number) => void
  data: ResponseIBTokOutlineSubmissionStatusDto[]
  type?: 'SUBMITTED' | 'REJECT_PLAN'
  title: string
  buttonText?: string
  buttonHandler?: () => void
  className?: string
...
export default function OutlineOverviewPanel(props: OverviewPanelProps) {
    ...
    return studentsToDisplay.reduce(
      ...
      {} as Record<
        string,
        {
          ...
          grade: number
          klass: number
          students: ResponseIBTokOutlineSubmissionStatusDto[]
        }
      >,
    )
  }, [studentsToDisplay])
        ...
        return (
          <div key={groupKey}>
            ...
            <div className="grid grid-cols-6 gap-3">
              {group.students
                .sort(
                  (a, b) => Number(a.leader.studentGroup.studentNumber) - Number(b.leader.studentGroup.studentNumber),
                )
                .map((student) => (
                  <div
                    key={student.id}
                    ...
                    onClick={goDetailPage ? () => goDetailPage(student.id, student.tokOutline.id) : undefined}
                  >
