...
import { ResponseIBProposalSubmissionStatusDto } from '@/legacy/generated/model'

interface OverviewPanelProps {
  goDetailPage?: (studentIbId: number, proposalId?: number) => void
  data: ResponseIBProposalSubmissionStatusDto[]
  type?: 'SUBMIT' | 'REJECT_PLAN'
  title: string
  buttonText?: string
  buttonHandler?: () => void
  className?: string
...
export default function ProposalOverviewPanel(props: OverviewPanelProps) {
  ...
  const studentsToDisplay = useMemo(() => {
    ...
    switch (type) {
      case 'SUBMIT':
        return data.filter((student) => student.detailStatus === 'SUBMIT')
      case 'REJECT_PLAN':
        return data.filter((student) => student.detailStatus === 'REJECT_PLAN')
    }
  }, [data, type])
    ...
    return studentsToDisplay.reduce(
      ...
      {} as Record<
        string,
        {
          ...
          grade: number
          klass: number
          students: ResponseIBProposalSubmissionStatusDto[]
        }
      >,
    )
  }, [studentsToDisplay])

  const approvedProposalId = (proposals: ResponseIBProposalSubmissionStatusDto['proposals']) => {
    return proposals.find((proposal) => proposal.status === 'ACCEPT')?.id || undefined
  }
            ...
            {group.students
              ...
              .map((student) => (
                <div
                  ...
                  onClick={
                    goDetailPage ? () => goDetailPage(student.id, approvedProposalId(student.proposals)) : undefined
                  }
                >
