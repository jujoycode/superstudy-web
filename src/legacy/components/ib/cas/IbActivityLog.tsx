...
import { useActivityLogCreate } from 'src/container/ib-cas';
import { RequestIBActivityLogDto, UploadFileTypeEnum } from 'src/generated/model';
...
export function IbActivityLog({
  modalOpen,
  setModalClose,
  projectId,
  onSuccess,
  ablePropragation = false,
}: PropsWithChildren<IbActivityLogProps>) {
  ...
  const onSubmit = async (data: RequestIBActivityLogDto) => {
    ...
    try {
      ...
      if (projectId !== undefined) {
        createActivityLog({ ibId: projectId, data: _data });
      }
    } finally {
    ...
    >
      ...
      <div className={`relative w-[848px] overflow-hidden rounded-xl bg-white px-8`}>
        <div className="sticky top-0 z-10 flex h-[88px] items-center justify-between bg-white/70 pb-6 pt-8 backdrop-blur-[20px]">
          <Typography variant="title1">활동일지 작성</Typography>
